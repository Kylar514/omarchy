...
grep -qF 'mise current "claude" &>/dev/null || mise use -g --quiet "claude" || exit 1' "$bin_dir/claude" ||
  fail "migration moves a stale wrapper to the current quiet template"
pass "migration moves a stale wrapper to the current quiet template"

grep -qF 'mise current "github:can1357/oh-my-pi" &>/dev/null || mise use -g --quiet "github:can1357/oh-my-pi" || exit 1' "$bin_dir/omp" ||
  fail "migration keeps a wrapper's package when the command name differs"
grep -qF 'exec mise exec -- "omp" "$@"' "$bin_dir/omp" ||
  fail "migration keeps a wrapper's bin name when the command name differs"
pass "migration preserves package and bin names"

grep -qF 'mise current "npm:@kitlangton/ghui" &>/dev/null' "$bin_dir/ghui" ||
  fail "migration preserves a scoped npm package name"
pass "migration preserves a scoped npm package name"

grep -qF 'mise current "github:someone/custom-tool" &>/dev/null || mise use -g --quiet "github:someone/custom-tool" || exit 1' "$bin_dir/custom-tool" ||
...
grep -qF 'mise current "npm:some/tool" &>/dev/null || mise use -g --quiet "npm:some/tool" || exit 1' "$bin_dir/mise-exec-era" ||
  fail "migration rewrites a wrapper on the mise-exec template"
grep -qF 'exec mise exec -- "tool-bin" "$@"' "$bin_dir/mise-exec-era" ||
  fail "migration keeps the bin name from a mise-exec wrapper"
grep -qF 'mise current "aqua:some/other" &>/dev/null || mise use -g --quiet "aqua:some/other" || exit 1' "$bin_dir/bare-exec-era" ||
  fail "migration rewrites a wrapper on the bare-exec template"
grep -qF 'exec mise exec -- "other-bin" "$@"' "$bin_dir/bare-exec-era" ||
